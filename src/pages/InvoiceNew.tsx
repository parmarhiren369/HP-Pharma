...
import { collection, doc, getDocs, runTransaction, setDoc, Timestamp } from "@/lib/firestore-shim";
...
export default function InvoiceNew() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only deduct inventory if status is "Approved"
      if (formData.status === "Approved") {
        ...
        // If not approved, just save the invoice without deducting inventory
        await setDoc(invoiceRef, payload);
      }

      toast({ title: "Saved", description: `Invoice saved${formData.status === "Approved" ? " and inventory deducted" : " (inventory not deducted until approved)"}.` });
      navigate("/invoices");
    } catch (error) {
