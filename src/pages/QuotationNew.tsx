...
type QuotationStatus = "Approved" | "In Process";
...
function generateSystemQuotationNo(date: string): string {
  const ymd = (date || new Date().toISOString().slice(0, 10)).replace(/-/g, "");
  const suffix = Math.floor(1000 + Math.random() * 9000);
  return `QUO-${ymd}-${suffix}`;
}
...
export default function QuotationNew() {
  ...
  useEffect(() => {
    setFormData((s) => {
      if (s.quotationNo) return s;
      return { ...s, quotationNo: generateSystemQuotationNo(s.issueDate) };
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.quotationNo.trim()) {
      toast({ title: "Validation error", description: "Quotation number is required.", variant: "destructive" });
      return;
    }

    if (!formData.manualQuotationNo.trim()) {
      toast({ title: "Validation error", description: "Manual quotation number is required.", variant: "destructive" });
      return;
    }
    ...
    const sanitizedItems: QuotationLineItem[] = lineItems
      .map((it) => ({
    ...
    const payload = {
      quotationNo: formData.quotationNo.trim(),
      manualQuotationNo: formData.manualQuotationNo.trim(),
    ...
    try {
      ...
      const quotationRef = doc(quotationsCol);

      await runTransaction(db, async (tx) => {
        // Deduct processed inventory first (abort quotation if stock insufficient)
        ...
        tx.set(quotationRef, payload);
      });

      toast({ title: "Saved", description: "Quotation saved to Firestore." });
      navigate("/quotations");
    } catch (error) {
      console.error("Error saving quotation", error);
      const msg = error instanceof Error ? error.message : "Could not save quotation.";
  ...
  return (
    <>
      <AppHeader title="New Quotation" subtitle="Create quotation (full page)" />
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-semibold">Create New Quotation</div>
              <div className="text-sm text-muted-foreground">This will be stored in Firestore.</div>
            </div>
          </div>

          <Button variant="outline" className="gap-2" onClick={() => navigate("/quotations")} disabled={isSubmitting}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Card className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="quotationNo">System Quotation No</Label>
                <Input id="quotationNo" value={formData.quotationNo} readOnly disabled />
              </div>

              <div className="space-y-2">
                <Label htmlFor="manualQuotationNo">Manual Quotation No *</Label>
                <Input
                  id="manualQuotationNo"
                  value={formData.manualQuotationNo}
                  onChange={(e) => setFormData((s) => ({ ...s, manualQuotationNo: e.target.value }))}
                />
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="issueDate">Issue Date (Quotation)</Label>
                <Input id="issueDate" type="date" value={formData.issueDate} onChange={(e) => setFormData((s) => ({ ...s, issueDate: e.target.value }))} />
              </div>

              <div className="space-y-2">
                <Label htmlFor="validUntil">Valid Until</Label>
                <Input id="validUntil" type="date" value={formData.validUntil} onChange={(e) => setFormData((s) => ({ ...s, validUntil: e.target.value }))} />
              </div>
            </div>
          </Card>
                    ...
                    {lineItems.map((it, idx) => {
                      ...
                      return (
                        <TableRow key={idx}>
                          ...
                          <TableCell className="align-top">
                            <Select value={it.unit} onValueChange={(v) => setLineItems((prev) => prev.map((x, i) => (i === idx ? { ...x, unit: v } : x)))}>
          ...
          <Card className="p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Status</Label>
                <Select value={formData.status} onValueChange={(v) => setFormData((s) => ({ ...s, status: v as QuotationStatus }))}>
                  <SelectTrigger>
          ...
          <div className="flex items-center justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => navigate("/quotations")} disabled={isSubmitting}>
              Cancel
            </Button>
